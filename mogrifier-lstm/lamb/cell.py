...
def build_cell(model, num_layers, hidden_size,
    ...
    def make_lstm_column():
        ...
        if overlay_rank > 0:
            ...
            # 1/sparsity_ratio.
            sparse_initializer = tf.compat.v1.truncated_normal_initializer(
        ...
        if lstm_skip_connection:
            ...
            return tf.compat.v1.nn.rnn_cell.MultiRNNCell(layers)

    def make_rhn_column():
        init_params = collections.OrderedDict([
            ('B_c', {'initializer': tf.compat.v1.constant_initializer(forget_bias)}),
        ])
