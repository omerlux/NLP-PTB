...
# from tensorflow.contrib import util as contrib_util # changed it to tf.constant
...
class Dropout(snt_base.AbstractModule):
  ...
  def _ensure_keep_mask(self, x):
    if self._keep_mask is None or not self._share_mask:
      ...
      noise = tf.compat.v1.random_uniform(shape, dtype=x.dtype)
      self._keep_mask = (tf.compat.v1.floor(self._keep_prob + noise)
                         * (self._scaler / self._keep_prob))
      self._keep_mask.set_shape(x.get_shape())
    return self._keep_mask

  def _build(self, x):
    if tf.constant(self._keep_prob) == 1:     #contrib_util.constant_value(self._keep_prob) == 1:
      return x
    else:
...
class GaussianDropout(snt_base.AbstractModule):
  ...
  def _ensure_keep_mask(self, x):
    if self._keep_mask is None or not self._share_mask:
      ...
      stddev = tf.compat.v1.sqrt((1 - self._keep_prob) / self._keep_prob)
      self._keep_mask = tf.compat.v1.random_normal(shape, mean=1.0, stddev=stddev,
                                         dtype=x.dtype)
      self._keep_mask.set_shape(x.get_shape())
    return self._keep_mask
...
class DirichletDropout(snt_base.AbstractModule):
  ...
  def _ensure_keep_mask(self, x):
    if self._keep_mask is None or not self._share_mask:
      ...
      kf = tf.compat.v1.cast(k, tf.compat.v1.float32)
      alpha = self._keep_prob * (kf - 1.0) / ((1-self._keep_prob)*kf) - 1.0/kf
      dist = tfp.distributions.Dirichlet(tf.compat.v1.ones(shape=k) * alpha)
  ...
  def _build(self, x):
    if tf.constant(self._keep_prob) == 1:  # contrib_util.constant_value(self._keep_prob) == 1:
      ...
      return tf.compat.v1.cond(tf.compat.v1.equal(self._keep_prob, 1.0),
                     lambda: x,
...
class DriftingDropout(snt_base.AbstractModule):
  ...
  def _build(self, x, state):
    ...
    shape = tf.compat.v1.shape(x)
    noise = tf.compat.v1.random_uniform(shape, dtype=x.dtype)
    other_mask = tf.compat.v1.floor(self._keep_prob + noise)
    choice_noise = tf.compat.v1.random_uniform(shape, dtype=x.dtype)
    choice = tf.compat.v1.less(choice_noise, self._flip_prob)
    ...
    sum_ = tf.compat.v1.reduce_sum(prev_keep_mask, 1, keepdims=True)
    is_initializing = tf.compat.v1.equal(sum_, 0.0)

    self._keep_mask = tf.compat.v1.where(tf.compat.v1.logical_or(choice, is_initializing),
                               other_mask,
                               prev_keep_mask)
    self._time_step += 1
