...
_config_options = [
    ...
    ('activation_fn', 'string', 'tf.compat.v1.tanh'),
...
def initialize():
  ...
  if not _is_initialized[0]:
    ...
    if FLAGS.flags_as_dict:
      ...
      with tf.compat.v1.gfile.GFile(FLAGS.flags_as_dict, 'r') as f:
    ...
    if FLAGS.config_file:
      ...
      with tf.compat.v1.gfile.GFile(FLAGS.config_file, 'r') as f:
...
def _handle_hps_proto_file():
  if FLAGS.hps_proto_file:
    ...
    with tf.compat.v1.gfile.GFile(FLAGS.hps_proto_file) as f:
...
class Config(object):
  ...
  def save(self, filename):
    with tf.compat.v1.gfile.GFile(filename, 'w') as f:
      f.write(str(self))
