...
class LM(object):
  ...
  def _build_model(self, config):
    self.global_step_var = tf.compat.v1.Variable(
        tf.compat.v1.zeros([], tf.compat.v1.int64), name='global_step', trainable=False)
    self.learning_rate = tf.compat.v1.placeholder(
        tf.compat.v1.float32, shape=[], name='learning_rate')

    ## Input variables

    self.num_samples = tf.compat.v1.placeholder_with_default(
    ...
    if config.conditioning_separator:
      ...
      self.conditioning = tf.compat.v1.placeholder(
          dtype=tf.compat.v1.int64, shape=[config.max_time_steps, None],
          name='conditioning')
      self.conditioning_len = tf.compat.v1.placeholder(dtype=tf.compat.v1.int64, shape=[None],
    ...
    self.source = tf.compat.v1.placeholder(
        dtype=tf.compat.v1.int64, shape=[config.max_time_steps, None], name='source')
    self.source_len = tf.compat.v1.placeholder(dtype=tf.compat.v1.int64, shape=[None],
                                     name='source_len')
    ...
    self.target = tf.compat.v1.placeholder(
        dtype=tf.compat.v1.int64, shape=[config.max_time_steps, None], name='target')

    def maybe_create_dropout_placeholder(configured_dropout_rate, name):
      if configured_dropout_rate > 0.0:
        return tf.compat.v1.placeholder(tf.compat.v1.float32, shape=[], name=name)
      else:
        return None
    ...
    self.softmax_temperature = tf.compat.v1.placeholder_with_default(
        1.0, shape=[], name='softmax_temperature')

    ## Training

    embedding_initializer = tf.compat.v1.variance_scaling_initializer(
        scale=config.embedding_init_factor, mode='fan_out',
        distribution='truncated_normal')
    output_initializer = tf.compat.v1.variance_scaling_initializer(
        scale=config.output_init_factor, mode='fan_in',
        distribution='truncated_normal')
    batch_size = tf.compat.v1.shape(self.source)[1]

    last_hidden_size = utils.ensure_list(config.hidden_size)[-1]

    tb_h = tf.compat.v1.stack([config.max_time_steps*batch_size, last_hidden_size])
    t_b_v = tf.compat.v1.stack([config.max_time_steps, batch_size, config.vocab_size])
    t_bk_o = tf.compat.v1.stack([
        config.max_time_steps,
        batch_size*(config.mos_num_components or 1),
        config.output_embedding_size])
    tbk_o = tf.compat.v1.stack([
        config.max_time_steps*
        batch_size*(config.mos_num_components or 1),
        config.output_embedding_size])
    t_b0_s_v = tf.compat.v1.stack(
        [config.max_time_steps, tf.compat.v1.div(batch_size, self.num_samples),
         self.num_samples, config.vocab_size])

    if config.embed_once:
      with tf.compat.v1.variable_scope('im', initializer=embedding_initializer):
        embedding = tf.compat.v1.get_variable(
            'embedding', [config.vocab_size, config.input_embedding_size],
            initializer=embedding_initializer, dtype=tf.compat.v1.float32)
        if self.embedding_dropout is not None:
          embedding = tf.compat.v1.nn.dropout(
              embedding, 1-self.embedding_dropout,
              noise_shape=tf.compat.v1.stack([config.vocab_size, 1]))
        embedded_source = tf.compat.v1.nn.embedding_lookup(embedding, self.source)
        if self.token_dropout is not None:
          embedding = tf.compat.v1.nn.dropout(
              embedding, 1-self.token_dropout,
              noise_shape=tf.compat.v1.stack([config.max_time_steps, batch_size, 1]))
        if config.scale_input_embeddings:
          embedded_source *= tf.compat.v1.sqrt(tf.compat.v1.cast(config.input_embedding_size,
    ...
    def lm_1(cell, initial_state, inputs, input_lens, scope=None):
      ...
      if FLAGS.swap_memory:
        return tf.compat.v1.nn.dynamic_rnn(cell=cell, inputs=inputs,
                                 time_major=True,
                                 sequence_length=input_lens,
                                 initial_state=initial_state,
                                 swap_memory=FLAGS.swap_memory,
                                 dtype=tf.compat.v1.float32, scope=scope)
      else:
        return tf.compat.v1.nn.static_rnn(cell=cell, inputs=tf.compat.v1.unstack(inputs),
                                sequence_length=input_lens,
                                initial_state=initial_state,
                                dtype=tf.compat.v1.float32, scope=scope)
    ...
    def output_module_1(outputs):
      with tf.compat.v1.variable_scope('om', initializer=output_initializer):
        ...
        softmax_bias = tf.compat.v1.get_variable('bias', [1, config.vocab_size],
                                       initializer=tf.compat.v1.zeros_initializer(),
                                       dtype=tf.compat.v1.float32)
        def to_softmax(x, dropout=self.downprojected_output_dropout):
          if dropout is not None:
            if not config.shared_mask_dropout:
              x = tf.compat.v1.nn.dropout(x, 1.0-dropout)
            else:
              x = tf.compat.v1.reshape(x, t_bk_o)
              x = tf.compat.v1.nn.dropout(
                  x, 1.0-dropout,
                  # same mask for all time steps
                  noise_shape=[
                      1, batch_size*(config.mos_num_components or 1),
                      config.output_embedding_size])
              x = tf.compat.v1.reshape(x, tbk_o)
          return (
              self.softmax_temperature*
              (tf.compat.v1.matmul(x, softmax_weights,
                         transpose_b=softmax_weights_transpose) + softmax_bias))

        last_hidden_size = utils.ensure_list(config.hidden_size)[-1]
        outputs_t_b_h = tf.compat.v1.convert_to_tensor(outputs)
        if self.output_dropout is not None:
          if not config.shared_mask_dropout:
            outputs_t_b_h = tf.compat.v1.nn.dropout(
                outputs_t_b_h, 1.0-self.output_dropout)
          else:
            outputs_t_b_h = tf.compat.v1.nn.dropout(
                outputs_t_b_h, 1.0-self.output_dropout,
                noise_shape=[1, batch_size, last_hidden_size])
        outputs_tb_h = tf.compat.v1.reshape(outputs_t_b_h, tb_h)

        if config.mos_num_components == 0:
          if config.output_embedding_size == last_hidden_size:
            return (tf.compat.v1.reshape(to_softmax(outputs_tb_h, None), t_b_v),
                    outputs_t_b_h)
          else:
            ...
            return tf.compat.v1.reshape(logits_tb_v, t_b_v), outputs_t_b_h
        else:
    ...
    lm = tf.compat.v1.make_template('lm', lm_1)
    ...
    def make_conditioning():
      if config.embed_once:
        with tf.compat.v1.variable_scope('cond_im', initializer=embedding_initializer):
          embedding = tf.compat.v1.get_variable(
          ...
          if self.embedding_dropout is not None:
            embedding = tf.compat.v1.nn.dropout(
                embedding, 1-self.embedding_dropout,
                noise_shape=tf.compat.v1.stack([config.conditioning_vocab_size, 1]))
          embedded_source = tf.compat.v1.nn.embedding_lookup(embedding, self.conditioning)
          if self.token_dropout is not None:
            embedding = tf.compat.v1.nn.dropout(
                embedding, 1-self.token_dropout,
                noise_shape=tf.compat.v1.stack([config.max_time_steps, batch_size, 1]))
      ...
      conditioning_lm = tf.compat.v1.make_template('cond_lm', lm_1)
      initial_state = conditioning_cell.zero_state(batch_size, dtype=tf.compat.v1.float32)
    ...
    if not config.embed_once:
      cell = tf.compat.v1.nn.rnn_cell.EmbeddingWrapper(
    ...
    if config.conditioning_separator:
      ...
      with tf.compat.v1.variable_scope('lm_init'):
        self.initial_state = utils.trainable_initial_state(
            batch_size, cell.state_size)
    else:
      self.initial_state = cell.zero_state(batch_size, dtype=tf.compat.v1.float32)
    outputs, self.last_state = lm(
        cell, self.initial_state, sources, self.source_len)
    self.cell_outputs = tf.compat.v1.convert_to_tensor(outputs)

    if config.output_once:
      output_module = tf.compat.v1.make_template('om', output_module_1)
      ...
      output_module_per_step = tf.compat.v1.make_template('om', output_module_per_step_1)
      ...
      logits_, _ = tf.compat.v1.nn.dynamic_rnn(cell=output_cell,
                                     inputs=tf.compat.v1.convert_to_tensor(outputs),
                                     time_major=True,
    ...
    def average_samples():
      ...
      logits = tf.compat.v1.reshape(logits_, t_b0_s_v)
      if config.model_average == 'geometric':
        x = tf.compat.v1.reduce_sum(logits, axis=2, keepdims=True)
      elif config.model_average == 'arithmetic':
        log_probs = tf.compat.v1.nn.log_softmax(logits)
        x = tf.compat.v1.reduce_logsumexp(log_probs, axis=2, keepdims=True)
      else:
        assert False, 'Not implemented.'
      # x is t_b0_1_v, tile it to t_b0_s_v.
      x = tf.compat.v1.ones_like(logits) * x
      return tf.compat.v1.reshape(x, t_b_v)

    self.logits = tf.compat.v1.cond(tf.compat.v1.equal(self.num_samples, 1),
                          lambda: logits_,
                          average_samples)
  ...
  def _build_loss(self, config):
    ...
    self.xe_loss = tf.compat.v1.reduce_sum(self.xe_losses, axis=0)
    self.log_probs = tf.compat.v1.nn.log_softmax(self.logits)

    if config.l2_penalty == 0.0:
      self.l2_loss = 0.0
    else:
      self.l2_loss = tf.compat.v1.add_n(
          [tf.compat.v1.nn.l2_loss(var) for var in tf.compat.v1.trainable_variables()])
    if config.l1_penalty == 0.0:
      self.l1_loss = 0.0
    else:
      self.l1_loss = tf.compat.v1.add_n(
          [tf.compat.v1.reduce_sum(tf.compat.v1.abs(var)) for var in tf.compat.v1.trainable_variables()])
    if config.activation_norm_penalty == 0.0:
      self.activation_norm_loss = 0.0
    else:
      self.activation_norm_loss = tf.compat.v1.reduce_mean(
          # Sum over time to make values compatible with AWD-LSTM by Merity.
          tf.compat.v1.reduce_sum(tf.compat.v1.square(self.dropped_cell_outputs), axis=0))

    self.unregularized_loss = tf.compat.v1.reduce_mean(self.xe_loss)
    ...
    self.training_summary = tf.compat.v1.summary.merge(
    ...
    if config.accum_batch_size > -1:
      ...
      grad_accumulators = [
          tf.compat.v1.Variable(tf.compat.v1.zeros_like(trained_var.initialized_value()),
                      trainable=False)
          for trained_var in trained_vars]
      self.accumulate_grads = tf.compat.v1.group(*[
      ...
      with tf.compat.v1.control_dependencies([self.accumulated_training_update]):
        self.accumulated_training_update = tf.compat.v1.group(
            *[var.assign(tf.compat.v1.zeros_like(var)) for var in grad_accumulators])
