...
class NASCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def __init__(self, num_units, num_proj=None,
    ...
    if num_proj is not None:
      self._state_size = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(num_units, num_proj)
  ...
  def call(self, inputs, state):
    ...
    concat_w_m = tf.compat.v1.get_variable(
    ...
    m_matrix = tf.compat.v1.matmul(m_prev, concat_w_m)
    inputs_matrix = tf.compat.v1.matmul(inputs, concat_w_inputs)
    ...
    if self._use_biases:
      b = tf.compat.v1.get_variable(
          ...
          dtype=dtype)
      m_matrix = tf.compat.v1.nn.bias_add(m_matrix, b)

    # The NAS cell branches into 8 different splits for both the hiddenstate
    # and the input
    m_matrix_splits = tf.compat.v1.split(axis=1, num_or_size_splits=8,
                               value=m_matrix)
    inputs_matrix_splits = tf.compat.v1.split(axis=1, num_or_size_splits=8,
                                    value=inputs_matrix)
    ...
    if self._num_proj is not None:
      concat_w_proj = tf.compat.v1.get_variable(
          "projection_weights", [self._num_units, self._num_proj],
          dtype)
      new_m = tf.compat.v1.matmul(new_m, concat_w_proj)

    new_state = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(new_c, new_m)
