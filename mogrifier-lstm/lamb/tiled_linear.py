...
import six
import sonnet
from sonnet.src import base_test as snt_base
...
class AbstractTiledLinear(snt_base.AbstractModule):
  ...
  def _get_variable(self, name, shape,
    ...
    return tf.compat.v1.get_variable(name, shape=shape, dtype=self._dtype,
  ...
  def _declared_input_sizes(self):
    ...
    return tf.compat.v1.TensorShape(sizes)

  def _inferred_input_sizes(self, inputs):
    return tf.compat.v1.TensorShape([input_.get_shape().as_list()[-1]
                           for input_ in inputs])
    ...
    if self._weights is None:
      ...
      def tiled_initializer(shape, dtype=self._dtype, partition_info=None):
        ...
        for column in columns:
          ...
          column_values.append(tf.compat.v1.concat(values, axis=0))
        return tf.compat.v1.concat(column_values, axis=1)
    ...
    if self._biases is None:
      ...
      for output_name, output_size in self._output_name_and_sizes_:
        bias = self._get_variable(
            ...
            default_initializer=tf.compat.v1.zeros_initializer())
        biases.append(bias)
      self._biases = tf.compat.v1.concat(biases, 0)
    return self._biases
  ...
  def _build_tiled_linear(self, inputs, input_name_and_sizes,
    ...
    def split_output(output):
      if len(output_name_and_sizes) == 1:
        ...
        # This is a bit faster than several tf.compat.v1.slice calls.
        return tf.compat.v1.split(output, len(output_name_and_sizes), axis=1)
      else:
        outputs = []
        offset = 0
        for _, output_size in output_name_and_sizes:
          outputs.append(tf.compat.v1.slice(output, [0, offset], [-1, output_size]))
          offset += output_size
        return outputs

    weights = self._ensure_weights()
    if len(inputs) > 1:
      inputs = tf.compat.v1.concat(inputs, 1)
    if add_bias:
      biases = self._ensure_biases()
      return split_output(tf.compat.v1.nn.xw_plus_b(inputs, weights, biases))
    else:
      return split_output(tf.compat.v1.matmul(inputs, weights))
  ...
  def _build_tiled_linear(self, inputs, input_name_and_sizes,
    ...
    def make_weights_for_inputs():
      ...
      for input_name, input_size in input_name_and_sizes:
        ...
        rows.append(tf.compat.v1.concat(weights_from_input, 1))
      return rows
    ...
    def make_biases():
      ...
      for name, size in output_name_and_sizes:
        bias = self._get_variable('B_{}'.format(name), shape=[size],
                                  default_initializer=tf.compat.v1.zeros_initializer())
        biases.append(bias)
      return tf.compat.v1.concat(biases, 0)
    ...
    for input_, weights, (name, _) in zip(inputs, weights_for_inputs,
                                          input_name_and_sizes):
      s += utils.layer_norm(tf.compat.v1.matmul(input_, weights), [1], bias=0.0,
...
class SparseTiledLinear(AbstractTiledLinear):
  ...
  tf.compat.v1.SparseTensor.indices). Generally, tiles with the same sharing key
  ...
  def _build_tiled_linear(self, inputs, input_name_and_sizes,
    ...
    for output_name, output_size in output_name_and_sizes:
      ...
      for input_, (input_name, input_size) in zip(inputs, input_name_and_sizes):
        ...
        r += tf.compat.v1.sparse_tensor_dense_matmul(weight, input_, adjoint_b=True)
      r = tf.compat.v1.transpose(r)
      if add_bias:
...
# tf.compat.v1.get_variable) instead of being tied to tiling.
class OverlaidTiledLinear(TiledLinear):
  ...
  def _get_variable(self, name, shape,
    ...
    if len(shape) != 2:
      ...
      if (sharing_key in self._matrix_cache and
          not tf.compat.v1.get_variable_scope().reuse):
        scaler = super(OverlaidTiledLinear, self)._get_variable(
            's_'+name, [shape[1]], default_initializer=tf.compat.v1.ones_initializer())
  ...
  def _low_rank_matrix(self, name, rank=None, shape=None,
    ...
    return tf.compat.v1.matmul(a, b)
