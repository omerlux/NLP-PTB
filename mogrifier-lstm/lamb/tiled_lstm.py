...
class TiledLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def _do_feature_masking(x, y, num_x, num_y, rounds, rank):
    for round_ in six.moves.range(rounds):
      ...
      if rank == 0:  # full rank case
        if transforming_x:
          x *= 2*tf.compat.v1.sigmoid(utils.linear(y, num_x, bias=True, scope=fm_name))
        else:
          y *= 2*tf.compat.v1.sigmoid(utils.linear(x, num_y, bias=True, scope=fm_name))
        ...
        bias = tf.compat.v1.get_variable(fm_name + '_bias', shape[1],
                               initializer=tf.compat.v1.zeros_initializer())
        if transforming_x:
          x *= 2*tf.compat.v1.sigmoid(tf.compat.v1.matmul(tf.compat.v1.matmul(y, a), b) + bias)
        else:
          y *= 2*tf.compat.v1.sigmoid(tf.compat.v1.matmul(tf.compat.v1.matmul(x, a), b) + bias)
    return x, y
  ...
  def __call__(self, input_, state, scope=None):
    ...
    with tf.compat.v1.variable_scope(scope or type(self).__name__,
      ...
      f = tf.compat.v1.sigmoid(f_pre)
      j = self._activation(j_pre)
      j = maybe_transform(self._update_transform, j)
      o = tf.compat.v1.sigmoid(o_pre)
      if self._tie_gates:
        c = f * state.c + (1-f) * j
      else:
        i = tf.compat.v1.sigmoid(i_pre)
        if self._cap_input_gate:
          c = f * state.c + tf.compat.v1.minimum(1-f, i) * j
        else:
          c = f * state.c + i * j
      ...
      if self._cell_clip is not None:
        ...
        c = tf.compat.v1.clip_by_value(c, -self._cell_clip, self._cell_clip)
    ...
    return h, tf.compat.v1.nn.rnn_cell.LSTMStateTuple(c, h)
