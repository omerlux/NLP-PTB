...
class TiledRHNCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, input_, state, scope=None):
    ...
    with tf.compat.v1.variable_scope(scope or type(self).__name__,
      ...
      for level in six.moves.range(self._depth):
        with tf.compat.v1.variable_scope('layer{}'.format(level)):
          ...
          c = tf.compat.v1.sigmoid(c_pre)
          h = self._activation(h_pre)
          h = maybe_transform(self._update_transform, h)
          if self._tie_gates:
            t = 1 - c
          else:
            t = tf.compat.v1.sigmoid(t_pre)
          s = c * s + t * h

          if self._cell_clip is not None:
            # pylint: disable=invalid-unary-operand-type
            s = tf.compat.v1.clip_by_value(s, -self._cell_clip, self._cell_clip)
