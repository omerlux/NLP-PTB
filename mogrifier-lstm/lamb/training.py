...
nest = tf.compat.v1.nest
FLAGS = flags.FLAGS


def _load_checkpoint(checkpoint_filename, extra_vars, trainable_only=False):
  if tf.compat.v1.gfile.IsDirectory(checkpoint_filename):
    checkpoint_filename = tf.compat.v1.train.latest_checkpoint(checkpoint_filename)
  logging.info('Loading checkpoint %s', checkpoint_filename)
  saveables = (tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) +
               tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.SAVEABLE_OBJECTS))
  if trainable_only:
    saveables = list(set(saveables) & set(tf.compat.v1.trainable_variables()))
  # Try to restore all saveables, if that fails try without extra_vars.
  try:
    saver = tf.compat.v1.train.Saver(var_list=saveables)
    saver.restore(tf.compat.v1.get_default_session(), checkpoint_filename)
  except (ValueError, tf.compat.v1.errors.NotFoundError):
    logging.info('Missing key in checkpoint. Trying old checkpoint format.')
    saver = tf.compat.v1.train.Saver(var_list=list(set(saveables) - set(extra_vars)))
    saver.restore(tf.compat.v1.get_default_session(), checkpoint_filename)
...
def train(tuner, data, vocab, config, experiment_dir, seed=None):
  ...
  Args:
    ...
    seed: suitable for tf.compat.v1.set_random_seed
  ...
  with tf.compat.v1.Graph().as_default():
    ...
    if FLAGS.trigger_averaging_turns >= 0:
      averaged = Averaged(tf.compat.v1.trainable_variables())
    else:
      averaged = None
    ...
    best_turn_saver = tf.compat.v1.train.Saver(max_to_keep=1, save_relative_paths=True)
    last_turn_saver = tf.compat.v1.train.Saver(max_to_keep=1, save_relative_paths=True)
    ...
    best_checkpoint_states = tf.compat.v1.train.get_checkpoint_state(best_checkpoint_dir)
    last_checkpoint_states = tf.compat.v1.train.get_checkpoint_state(last_checkpoint_dir)
    ...
    def maybe_save_checkpoint(saver, filename):
      if FLAGS.save_checkpoints:
        ...
        saver.save(tf.compat.v1.get_default_session(), filename,
