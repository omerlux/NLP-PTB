...
def linear(args, output_size, bias, bias_start=0.0, initializer=None,
           scope=None):
  with tf.compat.v1.variable_scope(scope or 'linear', initializer=initializer):
...
def _linear(args, output_size, bias, bias_start=0.0):
  ...
  with tf.compat.v1.variable_scope(scope) as outer_scope:
    ...
    if len(args) == 1:
      res = tf.compat.v1.matmul(args[0], weights)
    else:
      res = tf.compat.v1.matmul(tf.compat.v1.concat(args, 1), weights)
    if not bias:
      return res
    ...
    return tf.compat.v1.add(res, biases)
...
def linear_v2(args, output_size, bias, rank=None, scope=None,   # pylint: disable=missing-docstring
  ...
  with tf.compat.v1.variable_scope(scope or 'linear_v2'):
    if args is None or (nest.is_sequence(args) and not args):
      raise ValueError('`args` must be specified')
    if not nest.is_sequence(args):
      args = [args]
    ...
    with tf.compat.v1.variable_scope(scope) as outer_scope:
      ...
      if rank is None:
        ...
        if len(args) == 1:
          res = tf.compat.v1.matmul(tf.compat.v1.matmul(args[0], left), right)
        else:
          res = tf.compat.v1.matmul(tf.compat.v1.matmul(tf.compat.v1.concat(args, 1), left), right)
      if not bias:
        return res
      with tf.compat.v1.variable_scope(outer_scope) as inner_scope:
        inner_scope.set_partitioner(None)
        bias_initializer = find_initializer(var_init_params, bias_name, None)
        biases = tf.compat.v1.get_variable(bias_name, [output_size], dtype=dtype,
...
def layer_norm(x, reduction_indices, epsilon=1e-9, gain=None, bias=None,
  ...
  mean = tf.compat.v1.reduce_mean(x, reduction_indices, keep_dims=True)
  variance = tf.compat.v1.reduce_mean(tf.compat.v1.squared_difference(x, mean),
                            reduction_indices, keep_dims=True)
  normalized = (x - mean) / tf.compat.v1.sqrt(variance + epsilon)
  dtype = x.dtype
  ...
  with tf.compat.v1.variable_scope(scope or 'layer_norm'):
    if gain is None:
      gain = tf.compat.v1.get_variable('gain', shape=shape, dtype=dtype,
                             initializer=tf.compat.v1.ones_initializer())
    if bias is None:
      bias = tf.compat.v1.get_variable('bias', shape=shape, dtype=dtype,
...
def get_sparse_variable(name, indices, shape, dtype=None, trainable=True,
  ...
  n = len(indices)
  values = tf.compat.v1.get_variable(name, [n], dtype=dtype,
                           initializer=initializer, partitioner=partitioner,
                           regularizer=regularizer, trainable=trainable)
  return tf.compat.v1.sparse_reorder(
      tf.compat.v1.SparseTensor(indices=indices, values=values, dense_shape=shape))
...
def sparse_random_indices(ratio, shape):
  ...
  n = round_to_int(tf.compat.v1.TensorShape(shape).num_elements()*ratio)
  ...
  if ratio < 0.25:
    indices = {}
    if isinstance(shape, tf.compat.v1.TensorShape):
      shape = shape.as_list()
    while len(indices) < n:
...
def get_optimizer(optimizer_type):
  ...
  Returns:
    ...
    - a tf.compat.v1.HParams object from which hyperparamters for optimization
  ...
  if optimizer_type == 'adadelta':
    opt_func = tf.compat.v1.train.AdadeltaOptimizer
  elif optimizer_type == 'adagrad':
    opt_func = tf.compat.v1.train.AdagradOptimizer
...
def trainable_vars_in_scope(name):
  ...
  Returns:
    A list of tf.compat.v1.Variables in the scope.
  """
  vars_ = []
...
def create_grads(optimizer, loss, scopes, num_expected_missing_gradients=0):
  ...
  grads_and_vars = optimizer.compute_gradients(
      loss, ordered_vars,
      aggregation_method=tf.compat.v1.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N)
  ...
  for grad, var in grads_and_vars:
    summaries.append(
        tf.compat.v1.summary.scalar(escape_summary_name(var.name + '_grad_norm'),
                          tf.compat.v1.norm(grad)))
...
def clip_gradients_in_scope(grads_and_vars, scope, max_grad_norm):
  ...
  if max_grad_norm == 0:
    ...
    clipped_grads_in_scope, _ = tf.compat.v1.clip_by_global_norm(
...
def summaries_for_trainables():
  ...
  for var in tf.compat.v1.trainable_variables():
    ...
    summaries.append(tf.compat.v1.summary.scalar(name + '_mean', mean))
    summaries.append(tf.compat.v1.summary.scalar(name + '_var',
...
# tf.compat.v1.orthogonal_initializer is not stable numerically on some machines
# (gpus?).
def orthogonal_initializer(gain=1.0, dtype=tf.compat.v1.float32):
  ...
  def initialize(shape, dtype=dtype, partition_info=None, gain=gain):
    ...
    return tf.compat.v1.constant(gain*w.reshape(shape), dtype=dtype)
  return initialize


def random_mask(shape, k):
  x = tf.compat.v1.random_normal(shape=shape)
  kth_largest = tf.compat.v1.nn.top_k(x, k)[0][:, k-1]
  return tf.compat.v1.to_float(tf.compat.v1.greater_equal(x, tf.compat.v1.expand_dims(kth_largest, 1)))


def random_mask2(shape, k):
  x = tf.compat.v1.random_normal(shape=shape)
  x = tf.compat.v1.transpose(x)
  kth_largest = tf.compat.v1.nn.top_k(x, k)[0][:, k-1]
  mask = tf.compat.v1.to_float(tf.compat.v1.greater_equal(x, tf.compat.v1.expand_dims(kth_largest, 1)))
  return tf.compat.v1.transpose(mask)
...
class TFSerializer(object):
  ...
  def __init__(self, name='serialized', initializer='{}'):
    self._string = tf.compat.v1.get_variable(
        name, dtype=tf.compat.v1.string, trainable=False, initializer=initializer)
    self._new_string = tf.compat.v1.placeholder(dtype=tf.compat.v1.string, shape=[])
    self._assign_op = tf.compat.v1.assign(self._string, self._new_string)

  def store(self, obj):
    tf.compat.v1.get_default_session().run(self._assign_op,
                                 feed_dict={self._new_string: str(obj)})

  def retrieve(self):
    # pylint: disable=eval-used
    return eval(tf.compat.v1.get_default_session().run(self._string))
...
def mixture_of_softmaxes(x, k, e, to_logits):
  ...
  if True:  # pylint: disable=using-constant-test
    ...
    b = tf.compat.v1.shape(x)[0]
    p_b_ke = tf.compat.v1.tanh(linear(x, k*e, True, scope='projection'))
    p_bk_e = tf.compat.v1.reshape(p_b_ke, [b*k, e])
    log_mixture_weights_b_k = tf.compat.v1.nn.log_softmax(
        linear(x, k, False, scope='mos_weights'))
    log_mixture_weights_b_k_1 = tf.compat.v1.reshape(log_mixture_weights_b_k, [b, k, 1])
    logits_bk_v = to_logits(p_bk_e)
    logprobs_bk_v = tf.compat.v1.nn.log_softmax(logits_bk_v)
    logprobs_b_k_v = tf.compat.v1.reshape(logprobs_bk_v, [b, k, -1])
    logprobs_b_v = tf.compat.v1.reduce_logsumexp(
    ...
    p_b_ke = tf.compat.v1.tanh(linear(x, k*e, True, scope='projection'))
    p_bk_e = tf.compat.v1.reshape(p_b_ke, [b*k, e])
    mixture_weights_b_k = tf.compat.v1.nn.softmax(
        linear(x, k, False, scope='mos_weights'))
    mixture_weights_b_k_1 = tf.compat.v1.reshape(mixture_weights_b_k, [b, k, 1])
    logits_bk_v = to_logits(p_bk_e)
    probs_bk_v = tf.compat.v1.nn.softmax(logits_bk_v)
    probs_b_k_v = tf.compat.v1.reshape(probs_bk_v, [b, k, -1])
    probs_b_v = tf.compat.v1.reduce_sum(
        probs_b_k_v * mixture_weights_b_k_1,
        axis=1)
    return tf.compat.v1.log(probs_b_v+1e-8)


def expand_tile(tensor, n, name=None):
  """Returns a tensor repeated n times along a newly added first dimension."""
  with tf.compat.v1.name_scope(name, 'expand_tile'):
    n_ = tf.compat.v1.reshape(n, [1])
    num_dims = len(tensor.get_shape().as_list())
    multiples = tf.compat.v1.concat([n_, tf.compat.v1.ones([num_dims], dtype=tf.compat.v1.int32)], axis=0)
    # multiples = [n, 1, 1, ..., 1]
    res = tf.compat.v1.tile(tf.compat.v1.expand_dims(tensor, 0), multiples)
...
def get_batched_variable(name, runtime_batch_size, shape=None,
                         dtype=tf.compat.v1.float32, initializer=None,
                         trainable=True):
  ...
  shape = None if isinstance(initializer, tf.compat.v1.Tensor) else shape
...
def create_mask(lengths, max_length):
  ...
  return tf.compat.v1.less(tf.compat.v1.reshape(tf.compat.v1.range(max_length, dtype=lengths.dtype),
                            [-1, 1]),
                 tf.compat.v1.reshape(lengths, [1, -1]))
...
def mask_from_lengths(lengths, max_length=None, dtype=None, name=None):
  ...
  """
  with tf.compat.v1.name_scope(name, 'mask_from_lengths'):
    dtype = lengths.dtype if dtype is None else dtype
    max_length = tf.compat.v1.reduce_max(lengths) if max_length is None else max_length
    indexes = tf.compat.v1.range(max_length, dtype=lengths.dtype)
    mask = tf.compat.v1.less(tf.compat.v1.expand_dims(indexes, 0), tf.compat.v1.expand_dims(lengths, 1))
    cast_mask = tf.compat.v1.cast(mask, dtype)
  return tf.compat.v1.stop_gradient(cast_mask)
...
def compute_lengths(symbols_list, eos_symbol, name=None,
  ...
  Args:
    ...
    dtype: type of symbols, default: tf.compat.v1.int64.

  Returns:
    Tensor [batch_size] of lengths of sequences.
  """
  with tf.compat.v1.name_scope(name, 'compute_lengths'):
    max_len = len(symbols_list)
    eos_symbol_ = tf.compat.v1.constant(eos_symbol, dtype=dtype)
    # Array with max_len-time where we have EOS, 0 otherwise. Maximum of this is
    # the first EOS in that example.
    ends = [tf.compat.v1.constant(max_len - i, dtype=tf.compat.v1.int64)
            * tf.compat.v1.to_int64(tf.compat.v1.equal(s, eos_symbol_))
    ...
    # max_len+1 in lens_.
    lens_ = max_len + 1 - tf.compat.v1.reduce_max(tf.compat.v1.stack(ends, 1), axis=1)
    # For examples that didn't have EOS decrease max_len+1 to max_len as the
    # length.
    lens = tf.compat.v1.subtract(lens_, tf.compat.v1.to_int64(tf.compat.v1.equal(lens_, max_len + 1)))
    return tf.compat.v1.stop_gradient(tf.compat.v1.reshape(lens, [-1]))
...
def seq_softmax_cross_entropy_with_logits(logits, labels, lengths,
  ...
  """
  with tf.compat.v1.name_scope(name, 'seq_softmax_cross_entropy_with_logits',
    ...
    xe_terms = tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(
        logits=logits, labels=labels)
    masked_xe_terms = xe_terms * tf.compat.v1.cast(mask, xe_terms.dtype)
    if reduce_sum:
      return tf.compat.v1.reduce_sum(masked_xe_terms, axis=0)
    else:
...
def variance_scaling_initializer(scale=2.0, mode='fan_in',
                                 ...
                                 mean=0.0, seed=None, dtype=tf.compat.v1.float32):
  """Like tf.compat.v1.variance_scaling_initializer but supports non-zero means."""
  ...
  def _initializer(shape, dtype=dtype, partition_info=None):
    ...
    if distribution == 'truncated_normal':
      ...
      return tf.compat.v1.truncated_normal(shape, mean, trunc_stddev, dtype, seed=seed)
    elif distribution == 'uniform':
      # To get stddev = math.sqrt(scale / n) need to adjust for uniform.
      limit = math.sqrt(3.0 * scale / n)
      return tf.compat.v1.random_uniform(shape, mean-limit, mean+limit, dtype, seed=seed)
    else:
...
class FNCell(tf.compat.v1.nn.rnn_cell.RNNCell):
  ...
  def zero_state(self, batch_size, dtype):
    with tf.compat.v1.name_scope(type(self).__name__ + 'ZeroState', values=[batch_size]):
      return tf.compat.v1.zeros([batch_size, 1], dtype)
